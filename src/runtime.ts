...
export const [DEFAULT_REST_ENDPOINT, DEFAULT_SOCKET_ENDPOINT] = [
    'https://metrics-api.keiser.com/api',
    'wss://metrics-api.keiser.com/ws',
]
export const DEFAULT_REQUEST_TIMEOUT = 5000

export const BASE_PATH = "https://metrics-api.keiser.com/api".replace(/\/+$/, "");
...
export class MetricsConnection {
    ...
    try {
      const result = await this._retryStrategy.execute(async () => {
        return await new Promise((resolve, reject) => {
          ...
          const bodyRequiresRest = body instanceof FormData || body instanceof URLSearchParams
          if (this.socketConnected && !bodyRequiresRest) {
            const fullParams = {...params, ...this.flattenBody(body), ...pathParams}
            void this.actionSocket(action, fullParams, callback)
          } else {
            if(method.toUpperCase() === 'PATCH') {
              throw new ConnectionFaultError()
            }
            void this.actionRest(route, method, params, headers, body, callback)
          }
        })
      })
   ...
   */
  private flattenBody (body?: HTTPBody): Object {
    if (body === undefined || body === null) {
      return { }
    }
    if (Array.isArray(body) || body instanceof FormData || body instanceof URLSearchParams) {
      return { body }
    }
  ...
  private async actionRest (action: string, method: HTTPMethod, params: Object, headers: HTTPHeaders, body: HTTPBody | undefined, callback: (success: any, fail?: any) => void) {
    try {
      const upperMethod = method.toUpperCase()
      const methodSendsBody = upperMethod !== 'GET' && upperMethod !== 'HEAD'
      const response = await Axios({
        method: method,
        headers:headers,
        url: `${this._restEndpoint}${action}`,
        params: params,
        // Backward compat: when no body provided, non-GET/HEAD methods send params as body
        data: body ?? (methodSendsBody ? params : undefined),
...
export class BaseAPI {
    ...
    protected async request(context: RequestOpts, authenticated: boolean = false): Promise<any> {
        try {
          if (authenticated) {
            const response = await this.connection.authenticatedAction(context.action, context.path, context.method, context.query ?? {}, context.pathParams ?? {}, context.headers, context.body)
            return response as any
          } else {
            const response = await this.connection.action(context.action, context.path, context.method, context.query ?? {}, context.pathParams ?? {}, context.headers, context.body)
            return response as any
          }
        } catch (error) {
        throw error
...
export type Consume = { contentType: string };

export function canConsumeForm(consumes: Consume[] | undefined | null): boolean {
    if (!consumes) {
        return false;
    }
    for (const consume of consumes) {
        if ('multipart/form-data' === consume.contentType || 'application/x-www-form-urlencoded' === consume.contentType) {
            return true;
        }
    }
    return false;
